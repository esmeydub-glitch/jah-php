...
assert_true(!empty($gateway->getSalkIdentity()['signature']), 'GatewayAgent debe nacer con firma SALK.');

$observer = $engine->getAgent('ObserverAgent');
assert_true($observer instanceof Jah\Agents\ObserverAgent, 'ObserverAgent debe estar registrado.');
assert_true(
    $gateway->getSalkIdentity()['signature'] !== $observer->getSalkIdentity()['signature'],
    'Cada agente debe nacer con una firma SALK distinta.'
);

 ...
 *
 * Integrates with DataCoreTurbo (binary format) + MemoryPyramid (hot/warm/cold).
 * Exposes clean JSON to Qwen while maintaining internal binary storage.
 *
 * Endpoints:
 *   POST /api.php              → store/search/delete memory
 *   GET  /api.php?action=list  → list memories
 *   GET  /api.php?action=stats → tier statistics
 */
...
Autoloader::addNamespace('Jah\\DataCore\\',   __DIR__ . '/../jah-datacore/src');
...
$storagePath = $config['paths']['datacore_storage'] ?? __DIR__ . '/memory/datacore';
$hotStoragePath = $config['paths']['hot_storage'] ?? __DIR__ . '/memory/tiers/hot';
...
if ($method === 'GET') {
    ...
    switch ($action) {
        case 'list':
            ...
            $results = $turbo->query($collection, function (array $doc) use ($limit) {
    ...
    switch ($action) {
        ...
        case 'search':
            ...
            $results = $turbo->query($collection, function (array $doc) use ($queryTerms, $queryLower) {
                ...
                return $match || str_contains($searchable, $queryLower);
            });
